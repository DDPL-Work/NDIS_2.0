import { Bookmark, ExternalLink, Navigation, X } from 'lucide-react'
...
export default function GISInfoCard({ item, bookmarked = false, onClose, onBookmark, onOpen }) {
  if (!item) return null
  ...
  return (
    <div className="overflow-hidden rounded-xl border border-ink-200 bg-white shadow-lg">
      ...
      <div className="space-y-2.5 px-4 py-3 text-[12px]">
        ...
        {item.distanceM != null && <div className="flex items-center justify-between"><Info label="Distance" /><span>{(item.distanceM / 1000).toFixed(1)} km</span></div>}
