  ...
  return (
    ...
    >
      <legend className="px-1 text-[13px] font-semibold text-ink-900">
        {question.question}
        {question.required && <span className="ml-1 text-alert-600">*</span>}
      </legend>
      {["single_choice", "yes_no"].includes(question.type) && (
        <div
        ...
        >
          {Array.from(
            ...
            <button
              ...
              aria-label={`${rating} out of ${question.max || 5}`}
      ...
      {question.type === "optional_text" && (
        <div className="mt-3">
          ...
          <p className="mt-1 text-right text-[11px] text-ink-400">
            {String(value || "").length}/{question.maxLength || 300}
          </p>
        </div>
      )}
      {invalid && (
...
export default function CitizenFeedbackModal({ facility, open, onClose }) {
  ...
  useEffect(() => {
    ...
    backendFeedbackApi
      ...
      .then((sets) => {
        if (cancelled) return;
        const matched = Array.isArray(sets)
          ? sets.find((s) => (s.activeFrom ? new Date(s.activeFrom) <= new Date() : true))
          : null;
        setQuestions(matched?.questions || sets?.[0]?.questions || []);
      })
      .catch(() => {
        if (!cancelled) setQuestionsError("Could not load feedback questions.");
      })
      .finally(() => {
  ...
  const submit = async () => {
    ...
    if (missing.length) {
      ...
      requestAnimationFrame(() => {
        ...
        field?.querySelector("input, button, textarea")?.focus();
      });
      return;
    }
    setSubmitting(true);
        ...
        <div className="space-y-4">
          ...
          {questionsLoading && (
            <div className="flex items-center justify-center gap-2 py-8 text-sm text-ink-500">
              <Loader2 size={16} className="animate-spin" /> Loading feedback questions…
            </div>
          )}
          {questionsError && (
            <p className="rounded-lg border border-alert-200 bg-alert-50 p-3 text-[12px] text-alert-700" role="alert">
              {questionsError}
            </p>
          )}
          {!questionsLoading && !questionsError && questions.length === 0 && (
            <p className="py-6 text-center text-sm text-ink-500">
              No feedback questions available for this facility.
            </p>
          )}
