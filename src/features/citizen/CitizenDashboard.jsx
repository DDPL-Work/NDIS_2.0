...
export default function CitizenDashboard() {
  ...
  const stats = {
    active: value(data, ['active', 'active_count', 'pending']),
    review: value(data, ['awaiting_feedback', 'awaiting_verification', 'review']),
    escalated: value(data, ['escalated', 'escalated_count']),
    total: value(data, ['total', 'total_count', 'complaint_count']),
  }
  return <div className="space-y-6 max-w-5xl mx-auto p-6">
    ...
    <div className="grid grid-cols-1 sm:grid-cols-4 gap-4"><StatCard label="Active Complaints" value={loading ? '—' : stats.active} icon={Clock} tone="saffron" /><StatCard label="Waiting for Review" value={loading ? '—' : stats.review} icon={CheckCircle2} tone="leaf" /><StatCard label="Escalated" value={loading ? '—' : stats.escalated} icon={AlertTriangle} tone="alert" /><StatCard label="Total Complaints" value={loading ? '—' : stats.total} icon={FileText} tone="ink" /></div>
