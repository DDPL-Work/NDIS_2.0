...
export default function LoginPage({ initialMode = 'login' }) {
  ...
  function handleForgotPassword() { setShowForgotPassword(true) }
  function handleOtpSent(identifier) { setResetIdentifier(identifier); setShowForgotPassword(false); setShowResetPassword(true) }
...
return (
    <Fragment>
      <div className="min-h-screen bg-ink-950 flex items-center justify-center px-4 py-10">
        <div className="w-full max-w-lg">
          ...
          <div className="rounded-2xl border border-white/10 bg-white/[.06] p-7 shadow-2xl backdrop-blur">
            ...
            {!signup && (
              <button type="button" onClick={handleForgotPassword} className="w-full text-center text-xs text-saffron-300 hover:text-saffron-200 mt-2">
                <span className="flex items-center justify-center gap-1.5"><Mail size={11} /> Forgot password?</span>
              </button>
            )}
      ...
      <ResetPasswordModal isOpen={showResetPassword} onClose={() => { setShowResetPassword(false); setResetIdentifier('') }} initialIdentifier={resetIdentifier} />
    </Fragment>
  )
}
