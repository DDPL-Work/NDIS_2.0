...
// + complaint rollups run in parallel (independent lightweight calls, bounded
// by the number of departments) and are aggregated once. Stable deps
// (districtId + joined department ids) so nothing re-fires on render.
// Returns { rows, topGaps, facilityCount, avgGapScore }.
export function useDepartmentCoverage(districtId) {
  ...
  const { data, loading, error, refetch } = useAsync(async () => {
    ...
    const [facilityGroups, rollups] = await Promise.all([
    ...
    const rows = buildDepartmentRows(departments, facilityGroups, rollups)
    return {
      rows,
