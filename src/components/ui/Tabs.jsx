        ...
        return (
          <button
            ...
            className={clsx(
              ...
              active === value ? 'text-ink-950' : 'text-ink-500 hover:text-ink-800'
            )}
          >
            {tab.label}
            {tab.count !== undefined && <span className="ml-1.5 text-[11px] text-ink-400">{tab.count}</span>}
            {active === value && <span className="absolute left-2 right-2 -bottom-px h-[2px] rounded-full bg-saffron-500" />}
          </button>
        )
      })}
    </div>
