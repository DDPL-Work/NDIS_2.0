...
const mapEvidence = (item = {}) => {
  ...
  return {
    ...
    verificationStatus: item.verification_status || item.geotag_verification_status || null,
    exifAvailable: item.exif_available ?? item.has_exif_gps ?? null,
    photoGps: (item.exif_latitude != null && item.exif_longitude != null) ? [Number(item.exif_longitude), Number(item.exif_latitude)] : position,
    districtStatus: item.district_validation_status || item.boundary_status || null,
    verificationAt: item.verification_timestamp || item.verified_at || null,
    verificationSource: item.verification_source || null,
